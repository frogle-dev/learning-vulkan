...
[[nodiscard]]
AppResult<void> App::drawFrame()
{
    vk::Result result =
        logical_device_.waitForFences(1, &draw_fences_[frame_idx_], vk::True, UINT64_MAX);
...
[[nodiscard]]
AppResult<void> App::setupDebugMessenger()
{
    ...
    deletion_queue_.pushBack(
        [instance = instance_, debug_messenger = debug_messenger_](vk::Device device)
...
[[nodiscard]]
AppResult<void> App::createInstance()
{
...
[[nodiscard]]
AppResult<void> App::createWindowSurface()
{
    ...
    deletion_queue_.pushBack(
        [instance = instance_, window_surface = window_surface_](vk::Device device)
...
AppResult<bool> App::isDeviceSuitable(vk::PhysicalDevice const &physical_device)
{
    ...
    if (available_device_extensions.result != vk::Result::eSuccess)
        return AppError::unexpected({"Failed to enumerate device extension properties",
    ...
    {
        bool found = false;
        for (auto const &available_device_extension : available_device_extensions.value)
        {
...
[[nodiscard]]
AppResult<void> App::createLogicalDevice()
{
    ...
    {
        return AppError::unexpected(
            {"Couldnt find suitable queue family", AppErrorKind::NoSuitableQueueFamily});
    }
...
vk::SurfaceFormatKHR App::chooseSwapchainSurfaceFormat(
...
vk::PresentModeKHR App::chooseSwapchainPresentMode(
...
[[nodiscard]]
AppResult<vk::Extent2D> App::chooseSwapchainExtent(
...
[[nodiscard]]
AppResult<void> App::recreateSwapchain()
{
    int width    = 0;
    int height   = 0;
    bool success = SDL_GetWindowSizeInPixels(window_->getSDLWindow(), &width, &height);
...
AppResult<void> App::createSwapchain(bool recreate)
{
    ...
    result =
        logical_device_.createSwapchainKHR(&swapchain_create_info, nullptr, &swapchain_);
    if (result != vk::Result::eSuccess)
...
[[nodiscard]]
AppResult<vk::ImageView> App::createImageView(vk::Image const &image, vk::Format format,
                                              bool for_swapchain)
{
...
[[nodiscard]]
AppResult<void> App::createImageViews()
{
...
[[nodiscard]]
AppResult<void> App::createDescriptorSetLayout()
{
    std::array bindings = {
    ...
    deletion_queue_.pushBack(
        [descriptor_set_layout = descriptor_set_layout_](vk::Device device)
...
[[nodiscard]]
AppResult<void> App::createGraphicsPipeline()
{
    /* SHADER STAGE SETUP */
    ...
    vk::Result result = logical_device_.createPipelineLayout(&pipeline_layout_info,
...
[[nodiscard]]
AppResult<void> App::createCommandPool()
{
    ...
    deletion_queue_.pushBack([command_pool = command_pool_](vk::Device device)
...
[[nodiscard]]
AppResult<void> App::createImage(uint32_t width, uint32_t height, vk::Format format,
                                 ...
                                 vk::DeviceMemory &image_memory)
{
    ...
    deletion_queue_.pushBack(
        [image, image_memory](vk::Device device)
        {
...
[[nodiscard]]
AppResult<void> App::transitionImageLayout(
...
[[nodiscard]]
AppResult<void> App::createTextureImage()
{
    ...
    expected = copyBufferToImage(staging_buffer, texture_image_, tex_width, tex_height);

    if (!expected)
        return AppError::unexpected(expected.error());

    expected = transitionImageLayout(texture_image_, vk::ImageLayout::eTransferDstOptimal,
...
[[nodiscard]]
AppResult<void> App::createTextureImageView()
{
    auto expected = createImageView(texture_image_, vk::Format::eR8G8B8A8Srgb, false);
    if (!expected)
        return AppError::unexpected(expected.error());
...
[[nodiscard]]
AppResult<void> App::createTextureSampler()
{
    ...
    deletion_queue_.pushBack([tex_sampler = texture_sampler_](vk::Device device)
                             { device.destroySampler(tex_sampler); });

    return {};
}

[[nodiscard]]
AppResult<void> App::copyBufferToImage(vk::Buffer const &buffer, vk::Image &image,
...
[[nodiscard]]
AppResult<uint32_t> App::findMemoryType(uint32_t typeFilter,
...
[[nodiscard]]
AppResult<void> App::createBuffer(vk::DeviceSize size, vk::BufferUsageFlags usage,
                                  ...
                                  vk::DeviceMemory &buffer_memory)
{
    ...
    auto memory_type_idx = findMemoryType(mem_requirements.memoryTypeBits, properties);
    if (!memory_type_idx)
        return AppError::unexpected(memory_type_idx.error());
    ...
    deletion_queue_.pushBack(
        [buffer, buffer_memory](vk::Device device)
        {
...
[[nodiscard]]
AppResult<vk::CommandBuffer> App::beginOneTimeCommandBuffer()
{
...
[[nodiscard]]
AppResult<void> App::endOneTimeCommandBuffer(vk::CommandBuffer &command_buffer)
{
...
[[nodiscard]]
AppResult<void> App::copyBuffer(vk::Buffer &src_buffer, vk::Buffer &dst_buffer,
                                vk::DeviceSize size)
{
    ...
    copy_command_buf.value().copyBuffer(src_buffer, dst_buffer, 1, &buffer_copy_region);
...
[[nodiscard]]
AppResult<void> App::createVertexBuffer()
{
    ...
    void *data_staging;
    vk::Result result = logical_device_.mapMemory(staging_buffer_memory, 0, buffer_size,
                                                  {}, &data_staging);
...
[[nodiscard]]
AppResult<void> App::createIndexBuffer()
{
...
[[nodiscard]]
AppResult<void> App::createUniformBuffers()
{
    ...
    for (size_t i = 0; i < max_frames_in_flight; i++)
    {
        ...
        auto expected = createBuffer(buffer_size, vk::BufferUsageFlagBits::eUniformBuffer,
                                     vk::MemoryPropertyFlagBits::eHostVisible |
                                         vk::MemoryPropertyFlagBits::eHostCoherent,
                                     buffer, buffer_mem);
...
[[nodiscard]]
AppResult<void> App::createDescriptorPool()
{
    std::array pool_size{
    ...
    deletion_queue_.pushBack([descriptor_pool = descriptor_pool_](vk::Device device)
...
[[nodiscard]]
AppResult<void> App::createDescriptorSets()
{
    ...
    for (size_t i = 0; i < max_frames_in_flight; i++)
    {
        ...
        vk::DescriptorImageInfo image_info{.sampler   = texture_sampler_,
...
[[nodiscard]]
AppResult<void> App::createCommandBuffers()
{
    vk::CommandBufferAllocateInfo alloc_info{
        .commandPool        = command_pool_,
        .level              = vk::CommandBufferLevel::ePrimary,
...
[[nodiscard]]
AppResult<void> App::recordCommandBuffer(uint32_t image_idx)
{
    ...
    command_buffer.bindDescriptorSets(vk::PipelineBindPoint::eGraphics, pipeline_layout_,
                                      0, 1, &descriptor_sets_[frame_idx_], 0, nullptr);
...
[[nodiscard]]
AppResult<void> App::createSyncObjects()
{
    ...
    for (size_t i = 0; i < swapchain_images_.size(); i++)
    {
        ...
        vk::Result result = logical_device_.createSemaphore(&sphr_info, nullptr,
                                                            &render_finished_sphrs_[i]);

        if (result != vk::Result::eSuccess)
            return AppError::unexpected({"Failed to create semaphore", result});

        deletion_queue_.pushBack([sphr = render_finished_sphrs_[i]](vk::Device device)
                                 { device.destroySemaphore(sphr); });
    }
    ...
    for (size_t i = 0; i < max_frames_in_flight; i++)
    {
        ...
        result =
            logical_device_.createFence(&fence_create_info, nullptr, &draw_fences_[i]);

        if (result != vk::Result::eSuccess)
            return AppError::unexpected({"Failed to create fence", result});

        deletion_queue_.pushBack([sphr = present_complete_sphrs_[i]](vk::Device device)
                                 { device.destroySemaphore(sphr); });

        deletion_queue_.pushBack(
            [fence = draw_fences_[i]](vk::Device device) { device.destroyFence(fence); });
    }
